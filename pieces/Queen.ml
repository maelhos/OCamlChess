...
let queenLegalMoves (b: Board.t) (fs: int) : Move.t list =
  ...
  (long Bishop.slideTopRight.(fs) cps (long Bishop.slideTopLeft.(fs) cps (long Bishop.slideBottomRight.(fs) cps (long Bishop.slideBottomLeft.(fs) cps [])))))))
  end;;